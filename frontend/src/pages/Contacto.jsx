...
const WA_MSG = "Hola! Quisiera consultar por un producto.";
...
export default function Contacto() {
  ...
  const iconBox = (icon, green = false) => (
    <div
      className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
      style={{
        background: green ? "rgba(37,211,102,0.1)" : "var(--brand-tint)",
        color: green ? "#25D366" : "var(--brand)",
      }}
    >
      {icon}
    </div>
        ...
        <section className="bento p-6 space-y-5">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3">
              {iconBox(<Phone className="h-4 w-4" />)}
              <div>
                <p className="text-xs font-semibold mb-0.5 uppercase tracking-wide" style={{ color: "var(--muted)" }}>Teléfono</p>
            ...
            {whatsapp && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-semibold mb-0.5 uppercase tracking-wide" style={{ color: "var(--muted)" }}>WhatsApp</p>
                  <a href={waHref} target="_blank" rel="noreferrer"
                    className="inline-flex items-center gap-1.5 text-sm font-semibold px-3 py-1.5 rounded-xl text-white transition-all hover:scale-[1.02]"
            ...
            {email && (
              <div className="flex items-start gap-3">
                {iconBox(<Mail className="h-4 w-4" />)}
                <div>
                  <p className="text-xs font-semibold mb-0.5 uppercase tracking-wide" style={{ color: "var(--muted)" }}>Email</p>
                  <a href={`mailto:${email}`}
                    className="text-sm font-semibold hover:underline"
                    style={{ color: "var(--brand)" }}>
                    {email}
            ...
            {address && (
              <div className="flex items-start gap-3">
                {iconBox(<MapPin className="h-4 w-4" />)}
                <div>
                  <p className="text-xs font-semibold mb-0.5 uppercase tracking-wide" style={{ color: "var(--muted)" }}>Dirección</p>
                  <p className="text-sm leading-relaxed" style={{ color: "var(--text)" }}>{address}</p>
                </div>
              </div>
            )}
            ...
            {hours && (
              <div className="flex items-start gap-3">
                {iconBox(<Clock className="h-4 w-4" />)}
                <div>
                  <p className="text-xs font-semibold mb-0.5 uppercase tracking-wide" style={{ color: "var(--muted)" }}>Horario</p>
